...
def data_cleaning(data, lo, hi, use_spline = False, attr_list = ["Close**", "Volume", "Market Cap", "MarketShare"], window_len = 50, spline_num = 2):
    ...
    if use_spline:
        names = ["CryptoType", "Date", "Close**"] + [c + "." + s for c in attr_list for s in ([b + "." + str(a + 1) for a in range(spline_num) for b in ["Slope", "Length"]] + ["last"])]
    else:
...
def models_comparison(training_until="2019-01-01", test_size=100, epoch_lst=np.arange(11)*500, spline_num=5, n_cores=4, window_len=30):
    ...
    fname = "crypto_spline_until=" + training_until + "_spline=" + str(spline_num) + "_window=" + str(window_len) + ".csv"
...
#        plt.clf()
    fname_result = "crypto_results_until=" + training_until + "_spline=" + str(spline_num) + "_window=" + str(window_len) + ".csv"
    d = pd.DataFrame.from_dict(dct)
    d.to_csv(fname_result)
