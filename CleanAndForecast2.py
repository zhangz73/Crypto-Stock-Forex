...
def single_attr_spline(data, spline_num):
    ...
    index = len(data) - 1
    while index > 0:
        ret[idx * 3] = coef_tmp[int(dp_loss[idx, index, 1]), index]
        ret[idx * 3 + 1] = index - dp_loss[idx, index, 1] + 1
        ret[idx * 3 + 2] = ret[idx * 3] * ret[idx * 3 + 1]
        index = int(dp_loss[idx, index, 1])
        idx -= 1
        
    #return list(ret) + [data.iloc[-1]]
    return list(ret) + [data[-1]]
...
def data_cleaning(data, lo, hi, use_spline = False, attr_list = ["Close**", "Volume", "Market Cap", "MarketShare"], window_len = 50, spline_num = 2):
    ...
    if use_spline:
        names = ["CryptoType", "Date", "Close**"] + [c + "." + s for c in attr_list for s in ([b + "." + str(a + 1) for a in range(spline_num) for b in ["Slope", "Length", "Magnitude"]] + ["last"])]
    else:
...
def data_cleaning_parallel(data, use_spline = False, attr_list = ["Close**", "Volume", "Market Cap", "MarketShare"], window_len = 50, spline_num = 2, n_cores = 1):
    if n_cores <= 1:
        ...
        batch_size = int(math.ceil(len(data) / n_cores))
...
def training_forecasting(data, lstm_params = {"hidden_size": 1, "activations": ["relu"], "output_dim":[50], "epochs": 1000, "verbose":0}, training_until = "2019-01-01", test_size = 50, attr_list = ["Close**", "Volume", "Market Cap", "MarketShare"], use_spline = False, spline_num = 5, window_len = 100):
    ...
    for i in range(1, test_size + 1):
        ...
        crypto_close = model.predict(reshaping(test_set.iloc[i - 1], len(attr_list)), verbose=lstm_params["verbose"])[0]
...
def models_comparison(training_until="2019-01-01", test_size=50, epoch_lst=np.arange(11)*500, spline_num=5, n_cores=4, window_len=100):
    print("Cleaning Dataframe using stats...")
    crypto_stat = data_cleaning_parallel(crypto_info[-6000:-3000], use_spline=False, spline_num=spline_num, n_cores=n_cores, window_len=window_len)
    print("Cleaning Dataframe using spline...")
    crypto_spline = data_cleaning_parallel(crypto_info[-6000:-3000], use_spline=True, spline_num=spline_num, n_cores=n_cores)
    ...
    for crypto in cryptos:
        ...
        plt.ylabel("MSE")
        plt.legend()
        plt.savefig("comparison_plots2/" + crypto + ".png")
        plt.clf()

#models_comparison(training_until="2019-10-01")
print("Cleaning Dataframe using stats...")
crypto_stat = data_cleaning_parallel(crypto_info[-6000:-3000], use_spline=False, spline_num=5, n_cores=4, window_len=100)

train_mse_stat, test_mse_stat = training_forecasting(crypto_stat[crypto_stat["CryptoType"] == "Ethereum"], training_until="2019-10-01", test_size=50, lstm_params = {"hidden_size": 2, "activations": ["sigmoid", "tanh"], "output_dim":[50, 50], "epochs": 2000, "verbose":0}, use_spline=False, window_len=100)
print(train_mse_stat, test_mse_stat)

print("Cleaning Dataframe using spline...")
crypto_spline = data_cleaning_parallel(crypto_info[-6000:-3000], use_spline=True, spline_num=5, n_cores=4)

train_mse_spline, test_mse_spline = training_forecasting(crypto_spline[crypto_spline["CryptoType"] == "Ethereum"], training_until="2019-10-01", test_size=50, lstm_params = {"hidden_size": 2, "activations": ["sigmoid", "tanh"], "output_dim":[50, 50], "epochs": 2000, "verbose":0}, use_spline=True, spline_num=5, window_len=100)
print(train_mse_spline, test_mse_spline)
